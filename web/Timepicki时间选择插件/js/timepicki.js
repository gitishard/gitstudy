(function($) {
    $.fn.timepicki = function(options) {
        ...
        return this.each(function() {
            ...
            var ele_lef = ele.position().left+30;
            ele_hei += 15;
            $(ele).wrap("<div class='time_pick'>");
            var ele_par = $(this).parents(".time_pick");
            ele_par.append("<div class='timepicker_wrap'><div class='arrow_top'></div><div class='time'><div class='prev'></div><div class='ti_tx'></div><div class='next'></div></div><div class='mins'><div class='prev'></div><div class='mi_tx'></div><div class='next'></div></div><button class='submitTime' style='margin-left:130px;width:50px;background-color:#fff;' type='button'><b>OK</b></button></div>");
            // <div class='meridian'><div class='prev'></div><div class='mer_tx'></div><div class='next'></div></div>
            ...
            function(event) {
                 ...
                 //    $(".timepicker_wrap").siblings('input').focus();
                 //    ele_next.fadeOut()
                 // });
                    ...
                    if (!$(event.target).is(ele)) {
                        ...
                        if (tim.length != 0 && mini.length != 0) {
                           ...
                           ele.val(tim + " : " + mini )//+ " : "+ meri
                            $(".submitTime").click(function(){
                                ele_next.fadeOut()
                                // $(".timepicker_wrap").siblings().focus();
                                 $(".timepicker_wrap").prevAll().blur();
            ...
            function() {
                ...
                if (cur_ele.parent().attr("class") == "time") {
                    ...
                    if (cur_ele.attr("class") == "next") {
                        ...
                        if (cur_mins == 0) {
                            ele_next.find("." + cur_cli + " .mi_tx").text(50)//59
                        } else {
                            cur_mins-=10;//--
                            if (cur_mins < 10) {
                                ele_next.find("." + cur_cli + " .mi_tx").text("0" + cur_mins)
